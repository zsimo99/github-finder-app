...
import GithubContext from "../components/context/github/GuthubContext"
import { Link, useParams } from "react-router-dom"
import {FaStore,FaUserFriends, FaUsers, FaCode} from "react-icons/fa"
import Spinner from "../components/layout/Spinner"
import ReposList from "../components/Repos/ReposList"
import { getUsereAndRepos } from "../components/context/github/GithubActions"
function User() {
  const {user,loading,removeuser,repos,dispatch}=useContext(GithubContext)
  const params=useParams()

  useEffect(()=>{
     dispatch({type:"SET_LOADING"})
     const getUserData=async()=>{
      const userData=await getUsereAndRepos(params.login)
  ...
  return (
    ...
    <>
      <div className='w-full mx-auto lg:w-10/12'>
        <div className='mb-4'>
          <Link onClick={removeuser} to='/' className='btn btn-ghost'>
            Back To Search
          </Link>
        </div>
            ...
            <div className="w-full rounded-lg shadow-md bg-base-100 stats">
              ...
              {blog && (
                <div className="stat">
                  ...
                  <a href={`https://${blog}`} className="stat-value text-sm" target="_blank" rel="noreferrer">{blog}</a>
                </div>
              )}
              {twitter_username && (
                <div className="stat">
                  <div className="stat-title text-md">Twiter</div>
                  <a href={`https://twitter.com/${twitter_username}`} className="stat-value text-sm" target="_blank" rel="noreferrer">{twitter_username}</a>
                </div>
              )}
